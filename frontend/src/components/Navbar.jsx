...
function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 px-6 py-4 ${scrolled ? 'mt-0' : 'mt-2'}`}>
      <div className={`max-w-[1400px] mx-auto rounded-[32px] transition-all duration-500 border relative overflow-visible shadow-2xl ${scrolled ? 'bg-base-100/70 backdrop-blur-3xl border-white/10' : 'bg-transparent border-transparent'}`}>
        
        <div className="px-8 py-3 flex items-center justify-between relative z-10">
          
          {/* LOGO & MAIN NAV CLUSTER */}
            ...
            <div className="hidden xl:flex items-center gap-1">
              {[
                ...
                <Link
                  key={node.path}
                  to={node.path}
                  className={`flex items-center gap-2 px-5 py-3 rounded-2xl transition-all duration-500 font-black text-[11px] uppercase tracking-widest border border-transparent ${
                    isActive(node.path)
              ...
              {/* EXPLORE MEGA MENU (Moved back to center-left to resolve overlap) */}
              <div className="dropdown dropdown-hover ml-4">
                 ...
                 <ul tabIndex={0} className="dropdown-content z-[150] menu p-5 shadow-2xl bg-base-100/98 backdrop-blur-3xl rounded-[40px] w-80 border border-white/10 mt-2 gap-3">
                    <header className="px-4 py-2 border-b border-white/5 mb-2">
                       <span className="text-[10px] font-black uppercase tracking-[0.5em] text-primary">Neural Synchronization</span>
                    </header>
                    <div className="grid gap-1">
                      <li><Link to="/curated" className="rounded-2xl p-4 font-black text-xs hover:bg-primary/10 transition-colors">Curated Intelligence Tracks</Link></li>
                      <li><Link to="/whiteboard" className="rounded-2xl p-4 font-black text-xs hover:bg-primary/10 transition-colors">System Design Architecture</Link></li>
                      <li><Link to="/interview" className="rounded-2xl p-4 font-black text-xs bg-error/5 text-error hover:bg-error/10 transition-colors">AI Protocol Interview (LIVE)</Link></li>
                    </div>
                 </ul>
              </div>
            </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
               ...
               <div className="dropdown dropdown-end">
                  ...
                  <ul tabIndex={0} className="dropdown-content z-[200] menu p-4 shadow-3xl bg-base-100/98 backdrop-blur-3xl rounded-[32px] w-56 border border-white/10 mt-4 gap-1">
                     <header className="text-center py-2 mb-2 border-b border-white/5">
                        <span className="text-[9px] font-black uppercase tracking-widest opacity-40">INTERFACE_THEME</span>
                     </header>
                     <div className="grid grid-cols-1 gap-1">
                        {THEMES.map((theme) => (
                           <li key={theme}>
                              <button onClick={() => setCurrentTheme(theme)} className={`capitalize font-black text-[10px] rounded-xl py-2.5 justify-center ${currentTheme === theme ? "bg-primary text-primary-content" : "hover:bg-primary/5"}`}>
                                 {theme}
                              </button>
                           </li>
                        ))}
               ...
               <div className="ml-2 pl-4 border-l border-white/10 scale-90">
                  <UserButton appearance={{ elements: { userButtonAvatarBox: "size-10 rounded-2xl overflow-hidden shadow-2xl ring-1 ring-primary/20" } }} />
               </div>
            </div>
          </div>
        </div>
