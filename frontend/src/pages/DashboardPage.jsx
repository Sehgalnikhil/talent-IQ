...
function DashboardPage() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 font-sans pb-32 relative overflow-x-hidden ${isDark ? 'bg-base-300 text-base-content' : 'bg-base-300 text-base-content'}`}>
      <Navbar />
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pt-16 space-y-8">
        <header className="pt-4 pb-4 flex flex-col md:flex-row items-end justify-between gap-6 dashboard-item">
          ...
          <div className="flex items-center gap-4">
            <div className={`${isDark ? 'bg-base-content/5 border-base-content/10' : 'bg-base-100 border-base-200'} backdrop-blur-2xl border p-5 rounded-2xl flex items-center gap-10 shadow-xl`}>
                <div className="text-center">
                   <p className={`text-[9px] font-black uppercase tracking-widest mb-1 ${isDark ? 'text-base-content/40' : 'text-base-content/40'}`}>STREAK</p>
                   <p className="text-3xl font-black text-primary"><CountUp value={streak} />D</p>
                </div>

                <div className={`w-px h-10 ${isDark ? 'bg-base-content/10' : 'bg-base-content/10'}`} />
                <div className="text-center group cursor-pointer" onClick={() => navigate("/pricing")}>
                   <p className={`text-[9px] font-black uppercase tracking-widest mb-1 ${isDark ? 'text-base-content/40' : 'text-base-content/40'} group-hover:text-primary transition-colors`}>SCARLET</p>
        ...
        <div className="dashboard-item">
           <div className={`p-8 rounded-[40px] border relative overflow-hidden group transition-all duration-500 shadow-xl ${isDark ? 'bg-base-content/5 border-base-content/5' : 'bg-base-100 border-base-200'}`}>
              <div className={`absolute top-0 right-0 p-10 opacity-10 group-hover:opacity-20 transition-opacity ${isDark ? 'text-base-content' : 'text-primary'}`}>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
                 <div className="flex items-center gap-6">
                    ...
                    <div>
                       ...
                       <p className={`text-sm font-bold uppercase tracking-widest ${isDark ? 'text-base-content/40' : 'text-base-content/40'}`}>{dailyChallenge.problem.difficulty} • {dailyChallenge.hoursLeft}H REMAINING</p>
                    </div>
                 </div>
              ...
              <TiltCard onClick={() => navigate("/speedrun")} className={`p-12 border transition-all rounded-[50px] group cursor-pointer h-full relative overflow-hidden shadow-2xl ${isDark ? 'bg-gradient-to-br from-error/10 to-transparent border-error/20 hover:border-error/50' : 'bg-base-100 border-error/20 hover:border-error/50'}`}>
                 ...
                 <div className="flex items-center justify-between pointer-events-none relative z-10">
                    <div className="space-y-6">
                       ...
                       <h2 className={`text-5xl font-black tracking-tighter ${isDark ? 'text-base-content' : 'text-base-content'}`}>SPEEDRUN ARENA</h2>
                       <p className={`text-lg font-medium max-w-sm ${isDark ? 'text-base-content/40' : 'text-base-content/40'}`}>Match-ups live from the global consensus node.</p>
                    </div>
              ...
              <div className="size-40 bg-primary/5 absolute -top-10 -right-10 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-1000" />
                 <div className="flex items-center justify-between pointer-events-none relative z-10">
                    <div className="space-y-6">
                       ...
                       <h2 className={`text-5xl font-black tracking-tighter ${isDark ? 'text-base-content' : 'text-base-content'}`}>THE GAUNTLET</h2>
                       <p className={`text-lg font-medium max-w-sm ${isDark ? 'text-base-content/40' : 'text-base-content/40'}`}>Complete end-to-end interview Mastery Protocol.</p>
                    </div>
...
function ActivityPortfolio({ currentStreak, submissions, isDark }) {
    return (
        <TiltCard className={`p-10 h-full border rounded-[50px] shadow-2xl relative overflow-hidden ${isDark ? 'bg-base-content/5 border-base-content/5' : 'bg-base-100 border-base-200'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                <div className="space-y-6">
                    <p className={`text-7xl font-black ${isDark ? 'text-base-content' : 'text-base-content'}`}>{submissions.length}</p>
                    <p className={`text-[10px] font-black uppercase tracking-[0.4em] ${isDark ? 'text-base-content/40' : 'text-base-content/40'}`}>Process Synchronization</p>
                    <div className={`h-1 rounded-full overflow-hidden ${isDark ? 'bg-base-content/5' : 'bg-base-200'}`}>
                ...
                <div className={`flex flex-wrap gap-2 justify-end opacity-20 group-hover:opacity-40 transition-opacity`}>
                    {[...Array(60)].map((_, i) => (
                        <div key={i} className={`w-3 h-3 rounded-[2px] ${i % 4 === 0 ? 'bg-primary' : isDark ? 'bg-base-content/10' : 'bg-base-300'}`} />
                    ))}
                </div>
            </div>
        </TiltCard>
...
function AchievementsWidget({ badges, isDark }) {
    ...
    return (
        <TiltCard className={`p-10 h-full border rounded-[50px] shadow-2xl relative overflow-hidden ${isDark ? 'bg-base-content/5 border-base-content/5' : 'bg-base-100 border-base-200'}`}>
            ...
            <div className={`mt-12 pt-8 border-t flex justify-between items-center font-mono ${isDark ? 'border-base-content/5' : 'border-base-200'}`}>
                <span className={`text-[10px] font-black tracking-widest uppercase ${isDark ? 'text-base-content/40' : 'text-base-content/40'}`}>SYNC_LEVEL</span>
